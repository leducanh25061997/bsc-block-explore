...
import { useAddTransitionMutation, useSwapMutation } from '@/services/service';
  ...
  const handleBMToBNB = async (type: 'bnb-to-bm' | 'bm-to-bnb') => {
    ...
    if (type === 'bnb-to-bm') {
      try {
        ...
        const to = '0x30a8f76AaC428835c6632886A25c66Fa8216e7ae';
            ...
            },
            {
              onSuccess: () => {
                toast({
                  ...
                  description: `Transaction of ${bnbAmount} BNB completed successfully. You'll receive 0.68% BM token reward.`
                });
              },
              onError: error => {
                // console.log(error);
                    ...
                    <div className="bg-green-50 p-4 rounded-lg" data-id="u3uq7xuv6" data-path="src/components/SwapInterface.tsx">
                      <div className="flex items-center justify-between" data-id="r5a7tbe4o" data-path="src/components/SwapInterface.tsx">
                        ...
                        <span className="font-bold text-green-600" data-id="mf7td3dpw" data-path="src/components/SwapInterface.tsx">
                          {calculateBMFromBNB(bmAmount)} BNB
                          {/* {"100"} BM */}
                        </span>
                      </div>
                    </div>
